...
public class gameController implements Initializable {
    ...
    private static final String SERVER_IP = "192.168.137.1"; // 修改为服务器电脑的IP地址
...
//		System.out.println(playerFish.getFitWidth());
        ...
        allEnemy = new ImageView[]{
                ...
                enemyBoss5, enemyBoss6, enemyBoss7, enemy2Fish8
        };
...
//		System.out.println(currentOutfit);
    ...
    private void handleServerMessage(String message) {
        ...
        if (parts.length >= 3) {
            ...
            Platform.runLater(() -> {
                // 更新其他玩家的位置
                updateOtherPlayerPosition(playerName, x, y);
            });
        }
    }

    private Map<String, ImageView> otherPlayers = new HashMap<>();

    private void updateOtherPlayerPosition(String playerName, double x, double y) {
        if (!playerName.equals(currentName)) {  // 不更新自己的位置
            Platform.runLater(() -> {
                ImageView otherFish = otherPlayers.get(playerName);
                if (otherFish == null) {
                    // 创建新玩家的鱼
                    otherFish = new ImageView();
                    otherFish.setImage(new Image(gameController.class.getResourceAsStream("outfit0.png")));
                    otherFish.setFitWidth(width);
                    otherFish.setFitHeight(height);
                    otherPlayers.put(playerName, otherFish);
                    root.getChildren().add(otherFish);
                }
    ...
    private void sendPlayerPosition() {
        if (out != null) {
            String message = String.format("%s:%.2f:%.2f", currentName, playerFish.getX(), playerFish.getY());
            out.println(message);
        }
    }
    ...
    private void enemySwim(ImageView enemyF,
        ...
        TranslateTransition translate = new TranslateTransition(Duration.seconds(ori + s), enemyF);
    ...
    //detect the collision between enemy and player fish
    private void checkColl2(ImageView[] allEnemy) {

        Bounds playerB = playerFish.getBoundsInParent();
        arrayForEnemy();
        ...
        for (ImageView enemyF : allEnemy) {
            ...
            if (playerB.intersects(enemyBound)) {
...
//		System.out.println(height);
    ...
    private void scoreSystem(double num) {
        ...
        if (height >= enemy2Fish.getFitHeight() && !level2Run) {
            level2Run = true;
            level2();
            level = 2;
            levelUp(preH);
        ...
        if (height >= 5500 && !levelTwo3Run) {
            levelTwo3Run = true;
            levelTwo3();
        }
        if (height >= enemy3Fish.getFitHeight() && !level3Run) {
            level3Run = true;
            level3();
            level = 3;
            levelUp(preH);
        ...
        if (height >= 8500 && !levelThree4Run) {
            levelThree4Run = true;
            levelThree4();
        }
        if (height >= enemyBoss.getFitHeight() && !level4Run) {
            level4Run = true;
            level4();
            level = 4;
            levelUp(preH);
